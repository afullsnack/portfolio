...
import BaseLayout from '../../components/queststore/LayoutHOC';
      ...
      <Popover
         content={
            <Card bordered={false} bodyStyle={{
               ...
               display: 'flex',
               flexFlow: "column"
            }} actions={[<Button type="link" onClick={() => setVisibility(!visible)}>ADD TO CART</Button>]}>
...
class QuestStore extends Component {
   ...
   updateValues = ({ size, scroll, mouse }) => {
      if (size.changed){
        ...
        console.log(size.x, size.y);
         ...
         if(size.x <= 780) return this.setState({ slidesToShow: 1 });
         if(size.x <= 990) return this.setState({ slidesToShow: 2 });
         if(size.x <= 1720) return this.setState({ slidesToShow: 3 });
         if(size.x <= 2300) return this.setState({ slidesToShow: 4 });
      }
   }
      ...
      return(
         <Row gutter={0} style={{
            ...
            <Col span={24} style={{height: '100%', backgroundColor: '#eee'}}>
               <Carousel autoplay={false} draggable style={{height: '100%'}} afterChange={this.onChange} slidesToShow={this.state.slidesToShow} centerPadding='22px' centerMode={true}>
